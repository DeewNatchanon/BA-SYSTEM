...
function ManagerDashboard({ currentUser }) {
  ...
  // 🌟 จุดที่แก้ไข 1: เปลี่ยนค่า Default เป็นเวลาอัปเดตล่าสุด
  ...
  const handleConfirmApprove = async () => {
    ...
    if (selectedRequest.status === "Pending Approval" && !approvalData.assignee)
      return Swal.fire(
        "ข้อมูลไม่ครบ",
        "กรุณาระบุชื่อผู้รับผิดชอบ (Assignee) ก่อนอนุมัติ",
        "warning",
      );
        ...
        try {
          ...
          if (selectedRequest.status === "Pending Approval") {
            ...
            const approveData = {
              ...
              status: "Initiate",
              updated_at: new Date().toISOString(), // 🌟 จุดที่แก้ไข 2: บันทึกเวลาที่อนุมัติโปรเจกต์ใหม่
              form_data: {
                ...selectedRequest.form_data,
                assigned_to: approvalData.assignee, // มอบหมายผู้รับผิดชอบ IT ลงใน form_data
                compliance: {
            ...
            let approvedPercent =
              selectedRequest.form_data?.tracking?.completionPercent || 0;
            if (newStatus === "Go-live" || newPhase === "Go-live")
              ...
              newPhase === "Development/Implement" ||
            ...
            const updateData = {
              ...
              status: newStatus,
              phase: newPhase,
              updated_at: new Date().toISOString(), // 🌟 จุดที่แก้ไข 3: บันทึกเวลาที่อนุมัติเปลี่ยนสถานะ
              form_data: {
                ...selectedRequest.form_data,
        ...
        try {
          ...
          if (selectedRequest.status === "Pending Approval") {
            const finalData = {
              ...selectedRequest,
              status: "Rejected",
              updated_at: new Date().toISOString(), // 🌟 จุดที่แก้ไข 4: บันทึกเวลาที่ปฏิเสธโปรเจกต์ใหม่
              form_data: {
                ...selectedRequest.form_data,
                approval_remark: approvalData.remark,
              },
            ...
            if (
              selectedRequest.status === "Go-live" ||
              selectedRequest.phase === "Go-live"
            )
              revertedPercent = 100;
            else if (selectedRequest.phase === "Requirement")
              revertedPercent = 25;
            else if (selectedRequest.phase === "Preparation")
              revertedPercent = 50;
            else if (
              selectedRequest.phase === "Development/Implement" ||
              selectedRequest.phase === "Development"
            )
              revertedPercent = 75;
            ...
            const finalData = {
              ...selectedRequest,
              updated_at: new Date().toISOString(), // 🌟 จุดที่แก้ไข 5: บันทึกเวลาที่ปฏิเสธการเปลี่ยนสถานะ
              form_data: {
                ...selectedRequest.form_data,
                approval_remark: approvalData.remark,
                tracking: {
  ...
  const statusRequests = pendingRequests.filter(
    (r) =>
      r.status !== "Pending Approval" &&
      r.form_data?.tracking?.isPendingApproval,
  );
  const displayData = activeTab === "new" ? newRequests : statusRequests;

  const uniqueCategories = useMemo(() => {
    const cats = [
      ...new Set(newRequests.map((r) => r.category).filter(Boolean)),
    ];
  ...
  const uniqueStatuses = useMemo(() => {
    const statuses = [
      ...new Set(
        statusRequests
          .map((r) => r.form_data?.tracking?.pendingStatus)
          .filter(Boolean),
      ),
    ];
  ...
  const uniquePhases = useMemo(() => {
    const phases = [
      ...new Set(
        statusRequests
          .map((r) => r.form_data?.tracking?.pendingPhase)
          .filter(Boolean),
      ),
    ];
  ...
  // ย้ายฟังก์ชันจัดเรียงเข้ามาไว้ในนี้ เพื่อแก้ Warning ESLint
  const displayedRequests = useMemo(() => {
    let filtered = filterRows(displayData, {
      searchQuery: searchQuery,
      filters: {
        category:
          activeTab === "new" && filterCategory !== "All"
            ? filterCategory
            : null,
    ...
    if (activeTab === "status") {
      if (filterStatus !== "All") {
        filtered = filtered.filter(
          (r) => r.form_data?.tracking?.pendingStatus === filterStatus,
        );
      }
      if (filterPhase !== "All") {
        filtered = filtered.filter(
          (r) => r.form_data?.tracking?.pendingPhase === filterPhase,
        );
      }
    }
    ...
    const applySort = (data) => {
      ...
      return [...data].sort((a, b) => {
        // 🌟 จุดที่แก้ไข 6: สร้างเงื่อนไขเรียงลำดับเวลา (updated_at)
        ...
        if (sortBy === "pendingStatus") {
          aVal = a.form_data?.tracking?.pendingStatus || "";
          bVal = b.form_data?.tracking?.pendingStatus || "";
        }
        if (sortBy === "pendingPhase") {
          aVal = a.form_data?.tracking?.pendingPhase || "";
          bVal = b.form_data?.tracking?.pendingPhase || "";
        }
    ...
    <div className="page-wrap page-project">
      ...
      >
        <h1 className="page-heading" style={{ margin: 0 }}>
          {isCEO
            ? "Executive Dashboard (รายการรอพิจารณา)"
            : "Manager Dashboard (รออนุมัติ)"}
        </h1>
      </div>
      ...
      >
          ...
          >
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeTab === "new" ? "var(--card-bg)" : "transparent",
                color:
                  activeTab === "new" ? "var(--blue)" : "var(--text-muted)",
                fontWeight: activeTab === "new" ? 700 : 600,
                cursor: "pointer",
                transition: "all 0.2s",
                boxShadow:
                  activeTab === "new" ? "0 1px 3px rgba(0,0,0,0.05)" : "none",
                fontSize: "0.85rem",
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeTab === "status" ? "var(--card-bg)" : "transparent",
                color:
                  activeTab === "status" ? "var(--blue)" : "var(--text-muted)",
        ...
        >
          {isLoading ? (
            ...
            <table
              className="portfolio-table project-portfolio-table"
              style={{ width: "100%", borderCollapse: "collapse" }}
            >
              <thead>
                <tr>
                  {/* 🚀 แทนที่ด้วยหัวตารางที่กดได้ SortableHeader 🚀 */}
                  ...
                  {activeTab === "new" ? (
                    ...
                    <>
                      <SortableHeader
                        label="ขอเปลี่ยนเป็นสถานะ"
                        columnKey="pendingStatus"
                      />
                      <SortableHeader
                        label="ขอเปลี่ยนขั้นตอน (Phase)"
                        columnKey="pendingPhase"
                      />
                    </>
                  )}
                  <th
                    ...
                    {activeTab === "new" ? (
                      ...
                      <>
                        ...
                        >
                          ...
                          >
                            {request.form_data?.tracking?.pendingStatus}
                          </span>
                        </td>
                        <td
                          style={{
                        ...
                        >
                          ...
                          >
                            {request.form_data?.tracking?.pendingPhase}
                          </span>
                        </td>
                      </>
                    )}
        ...
        <div className="pdf-preview-overlay" style={{ zIndex: 1040 }}>
          ...
          >
            ...
            >
              ...
              >
                📝 ตรวจสอบและพิจารณา: {selectedRequest.name}
              </h3>
              <button
              ...
              >
                <h4
                  style={{
                    ...
                    boxShadow: "0 1px 0 rgba(14, 165, 233, 0.1)",
                    paddingBottom: "10px",
                    fontSize: "1.1rem",
                  }}
                >
                  📌 ข้อมูลจากผู้ขอ (Request Detail)
                </h4>
                <div
                  style={{
                    display: "grid",
                ...
                >
                  ...
                  >
                    ...
                    {selectedRequest.form_data?.requesterDept || "-"}
                  </div>
                  <div
                    style={{
                      background: "var(--bg-color)",
                  ...
                  >
                    ...
                    {selectedRequest.form_data?.expectedOutcome || "-"}
                  </div>
                </div>
                {selectedRequest.form_data?.tracking?.progressFile && (
                  ...
                  >
                    <strong style={{ color: "var(--blue)" }}>
                      📎 ไฟล์หลักฐานที่พนักงานแนบมา:
                    </strong>
                    <br />
                    <a
                      href={`http://localhost:4000/${selectedRequest.form_data.tracking.progressFile.replace(/\\/g, "/")}`}
                      target="_blank"
                      rel="noreferrer"
                      style={{
                        color: "var(--blue)",
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (selectedRequest.document_path)
                        window.open(
                          `http://localhost:4000/${selectedRequest.document_path.replace(/\\/g, "/")}`,
                          "_blank",
                        );
                      else
                        Swal.fire(
                  ...
                  <>
                    <h4
                      style={{
                        ...
                        boxShadow: "0 1px 0 rgba(16, 185, 129, 0.1)",
                        paddingBottom: "10px",
                        fontSize: "1.1rem",
                      }}
                    >
                      ✅ ส่วนการพิจารณาและสั่งการ
                    </h4>
                    {activeTab === "new" ? (
                      <>
                        <div
                        ...
                        >
                          ...
                          >
                            เริ่มงานใน Phase ไหน?
                          </label>
                          <select
                            value={approvalData.phase}
                            onChange={(e) =>
                              setApprovalData({
                                ...approvalData,
                          ...
                          >
                            <option value="Requirement">
                              Requirement (รับความต้องการเพิ่ม)
                            </option>
                            <option value="Development">
                              Development (พร้อมพัฒนาเลย)
                            </option>
                        ...
                        >
                          ...
                          >
                            {approvalData.manDay}{" "}
                            <span style={{ fontSize: "1rem" }}>วัน</span>
                          </strong>
                        </div>
                      </>
                    ) : (
                      ...
                      >
                        ...
                        >
                          {selectedRequest.form_data.tracking.pendingStatus}
                        </h2>
                        <p
                          style={{
                            margin: "5px 0 0 0",
                        ...
                        >
                          Phase:{" "}
                          {selectedRequest.form_data.tracking.pendingPhase}
                        </p>
                      </div>
                    )}
                    <div className="form-group">
                      ...
                      >
                        หมายเหตุ / ข้อสั่งการ (จะแสดงให้พนักงานเห็น)
                      </label>
                      <textarea
                        rows="3"
                        placeholder="ระบุเหตุผลในการปฏิเสธ หรือคำสั่งการเพิ่มเติม..."
